                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @if(! $user->hasRole('super_admin'))
                            <form action="{{ route('admin.pengguna.toggle-status', $user) }}" method="POST" class="d-inline" id="toggle-form-{{ $user->id }}">
                              @csrf
                              @method('PATCH')
                              @if($user->status === 'aktif')
                                <button type="button" class="btn btn-danger btn-sm" title="Nonaktifkan" onclick="confirmToggleStatus({{ $user->id }}, 'nonaktif', '{{ $user->username }}')">
                                  <i class="fas fa-ban"></i>
                                </button>
                              @else
                                <button type="button" class="btn btn-success btn-sm" title="Aktifkan" onclick="confirmToggleStatus({{ $user->id }}, 'aktif', '{{ $user->username }}')">
                          ...
                          @if(auth()->user()->hasRole('super_admin') && ! $user->hasRole('super_admin'))
                            <form action="{{ route('admin.pengguna.hapus', $user) }}" method="POST" class="d-inline" id="delete-form-{{ $user->id }}">
                              @csrf
                              @method('DELETE')
                              <button type="button" class="btn btn-danger btn-sm" title="Hapus" onclick="confirmDelete({{ $user->id }}, '{{ $user->username }}')">
